...
@section('content')
  ...
  <div class="card">
    <div class="card-header border-bottom">
      <h5 class="card-title mb-3">Services List</h5>
      ...
      <div class="row">
        ...
        <div class="dt-buttons btn-group flex-wrap" style="float: right"><button class="btn btn-secondary add-new btn-primary mx-3" tabindex="0" aria-controls="DataTables_Table_0" type="button"
          ...
          data-bs-target="#addNewCCModal"><span><i class="ti ti-plus me-0 me-sm-1 ti-xs"></i><span class="d-none d-sm-inline-block">Add New Service</span></span></button> </div>
        </div>
      </div>
    </div>
        ...
        <tbody>
          @foreach ($services as $service)
            <tr>
              ...
              <td><img src="{{ asset('admin-asset/images/service_images/' . $service->image) }}" width="50" height="50" alt=""></td>
              <td>
                <div class="d-flex align-items-center">
                  <a href="javascript:;" class="text-body edit-record" data-id="{{ $service->id }}"><i class="ti ti-pencil ti-sm mx-2"></i></a>

                 <a href="{{ route('service.serviceDelete', ['id' => $service->id]) }}" class="text-body delete-record"  onclick="return confirm('Are you sure you want to delete this record?')"><i class="ti ti-trash ti-sm mx-2"></i></a>
...
Dropzone.options.myDropzone = {
  ...
  $(document).on('click', '.edit-record', function() {
    ...
    var url = "{{ route('service.serviceEdit', ['id' => ':id']) }}";
  ...
  $(document).on('submit', '#addNewForm', function(event) {
    ...
    $.ajax({
        type: 'POST',
        url: '{{ route('service.serviceStore') }}',
        data: formData,
